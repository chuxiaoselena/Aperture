function [imdata, tsize] = init_scale_withInva(imdata, pa, step)
...
for n = 1:length(imdata)
    ...
    if isfield(imdata(n), 'outsideIm')
        cur_invalid = max(imdata(n).outsideIm,imdata(n).invisiable);
    end
    for p = 2:size(joints,1)        % p = 1 is root
        len(n,p-1) = norm(abs(joints(p,1:2)-joints(pa(p),1:2)));
        if isfield(imdata(n), 'outsideIm')
%             invalid(n,p-1) = max(cur_invalid(p) , cur_invalid(pa(p)));
            invalid(n,p-1) = cur_invalid(p) ;
            if (invalid(n,p-1)~=1 && len(n,p-1) == 0)
                invalid(n,p-1) = 1;
                fprintf( 'Warning: %d part %d->%d seems incorrect, modified as invalid\n', n, p, pa(p) );
                % discover error, update
                imdata(n).invalid(p) = 1;
                imdata(n).invalid(pa(p)) = 1;
            end
        end
    end
end
...
for i = 1:length(pa)-1
  ...
  ratio = ratio(~(invalid(:,1)==1 | invalid(:,i)==1));     % only valid len are used to compute ratio
